...
const SquireScreen = () => {
    ...
    return (
        <View>
            <ColorCounter
                onIncrease={() => dispatch({ type: 'change_red', payload: COLOR_COUNTER })}
                onDecrease={() => dispatch({ type: 'change_red', payload: -1 * COLOR_COUNTER })}
                color='red'
            />
            <ColorCounter
                onIncrease={() => dispatch({ type: "change_green", payload: COLOR_COUNTER })}
                onDecrease={() => dispatch({ type: 'change_green', payload: -1 * COLOR_COUNTER })}
                color='green'
            />
            <ColorCounter
                onIncrease={() => dispatch({ type: 'change_blue', payload: COLOR_COUNTER })}
                onDecrease={() => dispatch({ type: 'change_blue', payload: -1 * COLOR_COUNTER })}
                color='blue'
            />
            <View style={{ width: 150, height: 150, backgroundColor: `rgb(${red}, ${green}, ${blue})` }} />
        </View>
